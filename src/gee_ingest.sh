...
gcsURL=$3 # The path to the GCS URL folder that will hold CVSs for import to GEE
outP=$4 # The path to the local folder that holds CVSs for import to GCS
groupSize=$5
...
entIds=($(cat $genera_path))

echo "Extracted species:"
printf '%s\n' "${entIds[@]}"
...
do
  ...
  sql="select event_id as anno_id, lon, lat,
    ...
    strftime('%Y-%m-%dT%H:%M:%SZ', anno_hfp_datetime) as timestamp_hfp,
    	(row_number() over (order by lon, lat, timestamp)-1)/$groupSize as grp 
    from event 
    where genus = '${entId}'
    order by lon, lat, timestamp;"
